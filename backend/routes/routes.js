    ...
    async (req, res) => {
        try {
            if (req.user.isNewUser) {
                res.redirect('http://track-fitness-front.s3-website.eu-central-1.amazonaws.com/additional-info/additional-info.html?id=' + req.user._id);
            } else {
                res.redirect('http://track-fitness-front.s3-website.eu-central-1.amazonaws.com/profile/profile.html?id=' + req.user._id);
            }
        } catch (error) {
...
router.post('/additional-info', async (req, res) => {
    try {
        ...
        res.redirect(301, `http://track-fitness-front.s3-website.eu-central-1.amazonaws.com/profile/profile.html?id=${req.body.id}`);
    } catch (error) {
